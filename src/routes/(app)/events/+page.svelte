...
<Head title="Previous Event" path={PathEvents} />
        ...
        flex flex-col gap-5"
				>
					<div class="grid grid-cols-2 grid-rows-2 gap-4">
						{#each event.images as image (image)}
							<Image
								src={image}
								alt={event.title}
					...
					<div class="flex flex-col gap-3 justify-between">
						<div
							class="flex w-fit justify-center items-center bg-mnine-2 text-white px-4 py-1 rounded-xl"
						>
						...
						<div class="flex flex-col gap-3">
							...
							<div class="text-lg font-medium flex flex-row gap-2 items-center">
								<Icon src={TrOutlineBuildingCircus} size="25" className="shrink-0" />
								<span>Venue: {event.venue}</span>
							</div>
							<button
								class="bg-black hover:bg-black/80 text-white text-lg font-semibold py-2 px-10 rounded-xl w-fit
            flex flex-row gap-2 items-center justify-center
            cursor-pointer shadow-lg"
							>
								<span>More Portfolio</span>
								<Icon src={RiArrowsArrowRightUpLine} size="22" />
							</button>
						</div>
					</div>
				</div>
